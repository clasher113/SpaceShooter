...
Slider::Slider(Assets* assets, const sf::String& string, const Value& value, int min, int max) :
	...
	m_p_text(new sf::Text(*assets->getFont("ArialBlack"), string + std::visit([](auto& value) { return std::to_string(static_cast<int>(*value)); }, m_value)))
{
	...
	m_p_slider->setScale(windowAspectRatio);
	m_p_slider->setPosition(sf::Vector2f(calculatePosition(), m_p_shape->getPosition().y - 
							m_p_shape->getOutlineThickness() * windowAspectRatio.x * m_p_shape->getScale().x));
...
void Slider::input(const sf::Vector2f& cursorPos) {
	...
	if (Input::unclicked(sf::Mouse::Button::Left)) {
		m_pressed = false;
	}
}
...
void Slider::update(const float dt) {
	if (m_pressed) {
		...
		float left = m_p_shape->getGlobalBounds().position.x + m_p_slider->getGlobalBounds().size.x / 2.f + outlineThickness;
		float right = m_p_shape->getGlobalBounds().position.x + m_p_shape->getGlobalBounds().size.x - m_p_slider->getGlobalBounds().size.x / 2.f - outlineThickness;
...
void Slider::setPosition(const sf::Vector2f& position) {
	...
	m_p_slider->setPosition(sf::Vector2f(calculatePosition(), (position.y + outLine.y) - m_p_shape->getOutlineThickness() * m_p_shape->getScale().x));
